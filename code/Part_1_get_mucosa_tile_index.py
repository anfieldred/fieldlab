# -*- coding: utf-8 -*-
"""
Created on Mon Jan  1 18:52:27 2024
...
def open_img(slide_num):
   filename = os.path.join('d:\\img', str(slide_num).zfill(4)+'.png')   
...
def open_img_mark(slide_num):
    filename = os.path.join('d:\\img_mark', str(slide_num).zfill(4)+'.png')  
...
##读取保存在tile_index文件夹下的tile_index文件
##tile_index_slide_num=np.load(os.path.join('d:\\mucosa_tile_index',"{}.npy".format(str(slide_num).zfill(4))) )
...
def cut_slide_num_tiles(slide_num):
    slide_num_tiles = [] 
    for tile_index in np.load(os.path.join('d:\\mucosa_tile_index',"{}.npy".format(str(slide_num).zfill(4))) ):      
...
def get_slide_num_tile_names(slide_num):
  ...
  for seq in range(len(np.load(os.path.join('d:\\mucosa_tile_index',"{}.npy".format(str(slide_num).zfill(4))) ))):      
    tile_name = '{}_{}'.format(np.load(os.path.join('d:\\mucosa_tile_index',"{}.npy".format(str(slide_num).zfill(4))) )[seq][0],np.load(os.path.join('d:\\mucosa_tile_index',"{}.npy".format(str(slide_num).zfill(4))) )[seq][1])
...
tile_index_filename = os.path.join('d:\\mucosa_tile_index',"{}.npy".format(str(slide_num).zfill(4)))
np.save(tile_index_filename, img_tile_index_result)   
#tile_index_slide_num=list(np.load(os.path.join('d:\\mucosa_tile_index',"{}.npy".format(str(slide_num).zfill(4))) ))###读取第二次过滤结果坐标
...
out='d:\\mucosa_tile_png\\'+str(slide_num).zfill(4)+'\\' ##如'd:\\mucosa_tile_png\\0001\\'
