# -*- coding: utf-8 -*-
"""
Created on Sun May 19 19:00:14 2024

@author: Xin
"""
#TCGA generating samples
import math
import os
...
def cut_slide_num_tiles(slide_num):
    slide_num_tiles = [] 
    for tile_index in list(np.load(os.path.join('d:\\TCGA_final_tile_index',"{}.npy".format(str(slide_num).zfill(4))))):      
...
def get_slide_num_tile_names(slide_num):
  ...
  for seq in range(len(list(np.load(os.path.join('d:\\TCGA_final_tile_index',"{}.npy".format(str(slide_num).zfill(4))))))):      
    tile_name = '{}_{}'.format(list(np.load(os.path.join('d:\\TCGA_final_tile_index',"{}.npy".format(str(slide_num).zfill(4)))))[seq][0],list(np.load(os.path.join('d:\\TCGA_final_tile_index',"{}.npy".format(str(slide_num).zfill(4)))))[seq][1])
    slide_num_tile_names.append(tile_name)
  return slide_num_tile_names
##得到slide_num_list相应坐标的tile图片名称_list
def get_tile_names_list(starting,ending):
    tile_names_list=[]
    for slide_num in range(starting,ending):
        slide_num_tile_names=get_slide_num_tile_names(slide_num)
        tile_names_list.append(slide_num_tile_names)
...
###N_status_dataframe=pd.read_excel('d:\\data\\N_status.xlsx')  ##(N0,N1,N2,N3a,N3b)
N_status_dataframe=pd.read_excel('d:\\data\\TCGA_N_p_n.xlsx')   ##(Negative, Positive)
